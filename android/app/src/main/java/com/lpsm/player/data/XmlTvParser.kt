...
object XmlTvParser {
 fun current(xml:String,now:Date=Date()):Map<String,String> = current(StringReader(xml), now)
 fun current(reader:Reader,now:Date=Date()):Map<String,String>{val out=mutableMapOf<String,String>();val p=Xml.newPullParser();p.setInput(reader);var channel="";var start:Date?=null;var stop:Date?=null;var title="";while(p.eventType!=org.xmlpull.v1.XmlPullParser.END_DOCUMENT){if(p.eventType==org.xmlpull.v1.XmlPullParser.DOCDECL)throw IllegalArgumentException("DTD não permitido");if(p.eventType==org.xmlpull.v1.XmlPullParser.START_TAG){when(p.name){"programme"->{channel=p.getAttributeValue(null,"channel")?:"";start=date(p.getAttributeValue(null,"start"));stop=date(p.getAttributeValue(null,"stop"))};"title"->title=p.nextText()}}else if(p.eventType==org.xmlpull.v1.XmlPullParser.END_TAG&&p.name=="programme"){if(start!=null&&stop!=null&&!now.before(start)&&now.before(stop))out[channel]=title;channel="";title=""};p.next()};return out}
